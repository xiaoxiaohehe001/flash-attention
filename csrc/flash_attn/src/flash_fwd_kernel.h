...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    if (n_block_min == 0 || n_block_sink >= n_block_max){
        n_block_sink = 0;
    }
    #ifdef FLASH_DEBUG
    if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
    ...
    #ifdef FLASH_DEBUG
        if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
            printf("## 1st copy K ## %d-%d, n_block:%d, n_block_min:%d,  n_block_sink: %d, tKgK.data: %p, tVgV.data: %p \n",
            threadIdx.x,
            blockIdx.x,
            n_block,
            n_block_min,
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            #ifdef FLASH_DEBUG
                if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                    printf("## 2nd copy V masking ## %d-%d, n_block:%d, n_block_min:%d,  n_block_sink: %d, tKgK.data: %p, tVgV.data: %p \n",
                    threadIdx.x,
                    blockIdx.x,
                    n_block,
                    n_block_min,
            ...
            #ifdef FLASH_DEBUG
                if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                    printf("## 2nd copy V not masking ## %d-%d, n_block:%d, n_block_min:%d,  n_block_sink: %d, tKgK.data: %p, tVgV.data: %p \n",
                    threadIdx.x,
                    blockIdx.x,
                    n_block,
                    n_block_min,
        ...
        if (n_block > n_block_min - n_block_sink) {
            ...
            #ifdef FLASH_DEBUG
                if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                    printf("## 3nd copy K ## %d-%d, n_block:%d, n_block_min:%d,  n_block_sink: %d, tKgK.data: %p, tVgV.data: %p \n",
                    threadIdx.x,
                    blockIdx.x,
                    n_block,
                    n_block_min,
    ...
    for (; n_block >= n_block_min - n_block_sink; --n_block) {
        ...
        #ifdef FLASH_DEBUG
            if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                printf("## 4nd copy V ## %d-%d, n_block:%d, n_block_min:%d,  n_block_sink: %d, tKgK.data: %p, tVgV.data: %p \n",
                threadIdx.x,
                blockIdx.x,
                n_block,
                n_block_min,
        ...
        if (n_block > n_block_min - n_block_sink) {
            ...
            #ifdef FLASH_DEBUG
                if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
                    printf("## 5nd copy K ## %d-%d, n_block:%d, n_block_min:%d,  n_block_sink: %d, tKgK.data: %p, tVgV.data: %p \n",
                    threadIdx.x,
                    blockIdx.x,
                    n_block,
                    n_block_min,
        ...
        if(n_block >= n_block_min){
            ...
            mask_sink.template apply_mask_sink(
                acc_s, 
                params.sink_token_len,
                n_block * kBlockN
            );
